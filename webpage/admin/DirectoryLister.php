...
class DirectoryLister {
    ...
    public function zipDirectory($directory) {
        ...
        if ($this->_config['zip_dirs']) {
            ...
            $exclude_list = implode(' ', array_merge($this->_config['hidden_files'], array('file.php')));
...
//print_r($files);
    ...
    // if($value!="." && $value!="..")
    // {
        //print_r(file_get_contents($filePath."/".$value));

		// if($value!=$filePath."/*.lab")
    // {
	//$filePath = preg_replace('/[^a-z0-9\_\-\.]/i', '', $filePath);
		$filePath = preg_replace('"\.txt$"', '.lab', $filePath);
    ...
    protected function _readDirectory($directory, $sort = 'natcase') {
		...
		//$filesins = scandir($directory. "*.lab");
                ...
                if ($file == '..') {
                    ...
                    if ($this->_directory != '.') {
                        ...
                        $directoryArray['..'] = array(
                            ...
                            'url_path'   => $this->_appURL . 'file.php' . $directoryPath,
                            'file_size'  => '-',
							'label' => '-',
                    ...
                    if ($this->_directory != '.' || $file != 'index.php') {
                        ...
                        $directoryArray[pathinfo($relativePath, PATHINFO_BASENAME)] = array(
							...
							'label' => is_dir($realPath) ? '-' : $this->getFileLabel($realPath),
                            'mod_time'   => date($this->_config['date_format'], filemtime($realPath)),
